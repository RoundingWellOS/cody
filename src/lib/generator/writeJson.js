...
function groupByCodeLength(mappedList) {
    // @ts-ignore
    return _.omitBy(_.groupBy(mappedList, ({ code }) => code.length), (value, key) => !isValidIcdLength(Number(key)));
}
...
function createListWithCodeAssociations(mappedList) {
    ...
    return _.reduce(mappedList, (memo, entry) => {
...
function formatAllCodesWithPeriod(reducedList) {
    return _.map(reducedList, entry => {
        ...
        if (entry.children) entry.children = _.map(entry.children, child => {
